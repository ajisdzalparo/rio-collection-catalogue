...
const emptyArchive: ArchiveInput = {
  name: '',
  slug: '',
  description: '',
  imageUrl: '',
...
export default function ArchivesPage() {
    ...
    setForm(
      archive
        ? {
            ...
            journalId: archive.journalId ?? null
          }
        : emptyArchive
    );
    setFormError('');
  ...
  const columns: Column<ArchiveCollection>[] = [
    ...
    { header: 'Slug', accessorKey: 'slug' },
    {
      header: 'Artikel',
      cell: (item) =>
        journals.find((journal) => journal.id === item.journalId)?.title || 'Belum ditautkan'
    },
    {
      header: 'Aksi',
      cell: (item) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            className="w-40 rounded-2xl border-border/40 p-1.5 shadow-lg"
          >
  ...
  return (
    <section className="space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Historical Archive</h1>
          <p className="text-sm text-muted-foreground">
            Kelola koleksi arsip dan pilih artikel jurnal yang dituju.
          </p>
        </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingId ? 'Edit Arsip' : 'Tambah Arsip'}</DialogTitle>
            <DialogDescription>
              Perubahan akan tampil di katalog dan halaman Archive.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={submit} className="space-y-4">
            <div>
              <Label htmlFor="archive-name">Nama</Label>
              <Input
                id="archive-name"
                required
                value={form.name}
            ...
            <div>
              <Label htmlFor="archive-slug">Slug</Label>
              <Input
                id="archive-slug"
                required
            ...
            <div>
              <Label htmlFor="archive-description">Deskripsi</Label>
              <Textarea
            ...
            <div>
              <p className="text-sm font-medium">Gambar Arsip</p>
              <ImageUpload
                value={form.imageUrl}
                onChange={(imageUrl) => setForm({ ...form, imageUrl })}
                aspectRatio="3:4"
                placeholder="Upload foto arsip 3:4"
              />
            </div>
            <div>
              <Label htmlFor="archive-journal">Artikel Jurnal</Label>
              <Select
                value={form.journalId || 'NONE'}
                onValueChange={(val) => setForm({ ...form, journalId: val === 'NONE' ? null : val })}
              >
                <SelectTrigger id="archive-journal" className="w-full rounded-lg">
                  <SelectValue placeholder="Tanpa artikel — buka daftar jurnal" />
                </SelectTrigger>
                <SelectContent className="rounded-xl">
                  <SelectItem value="NONE">Tanpa artikel — buka daftar jurnal</SelectItem>
            ...
            <Button type="submit" disabled={save.isPending || !form.imageUrl}>
      ...
      <ConfirmModal
        ...
        title="Hapus arsip?"
        description={`Arsip ${deleting?.name || ''} akan dihapus dari katalog.`}
        isLoading={remove.isPending}
        onConfirm={() => {
          if (deleting)
            remove.mutate(deleting.id, {
              onSuccess: () => toast.success('Arsip dihapus'),
