...
export function MobileNav({ isOpen, onClose, storeName }: MobileNavProps) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      className="fixed inset-0 z-50 bg-(--cat-surface) flex flex-col animate-in fade-in-0 duration-200"
    >
      ...
      <nav className="flex-1 flex flex-col justify-center px-8 gap-8">
        {NAV_LINKS.map((link) => (
          <div key={link.href} className="animate-in slide-in-from-bottom-2 fade-in-0 duration-300">
            <Link
              href={link.href}
              onClick={onClose}
              className={cn(
